  ...
  return (
    ...
    >
      {imgUrl && !imgError ? (
        ...
        <div
          className={`w-full h-full flex items-center justify-center text-white font-semibold bg-gradient-to-br ${nameToGradient(
            name
          )}`}
        >
...
/* ---------- motion variants ---------- */
const rowVariant = { hidden: { opacity: 0, y: 8 }, visible: { opacity: 1, y: 0 }, hover: { scale: 1.01 } };
      ...
      try {
        ...
        const unsub2 = onSnapshot(
          ...
          (snap) => {
            ...
            toast.dismiss(); // clear any loading toast if any
          },
          (err) => {
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            ...
            <div className="flex items-center gap-3 w-full md:w-auto">
              <div className="flex items-center bg-white border rounded-full px-3 py-2 shadow-sm flex-1 md:flex-none min-w-[200px]">
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-slate-100">
              ...
              <table className="min-w-full text-sm text-slate-700">
                <thead className="sticky-thead">
                  <tr className="bg-slate-50 text-slate-500 uppercase text-xs tracking-wide border-b">
                    <th className="px-6 py-3 w-[45%] text-left">Customer</th>
                    <th className="px-6 py-3 w-[30%] text-left">Email</th>
                    <th className="px-6 py-3 w-[15%] text-left">Joined</th>
                  ...
                  {loading ? (
                    Array.from({ length: 6 }).map((_, i) => (
                      <tr key={i} className="border-b">
                        ...
                        <td className="px-6 py-5"><div className="h-3 w-48 bg-slate-200 rounded" /></td>
                        <td className="px-6 py-5"><div className="h-3 w-24 bg-slate-200 rounded" /></td>
                        <td className="px-6 py-5 text-center"><div className="h-8 w-24 bg-slate-200 rounded" /></td>
                      </tr>
                    ))
                  ) : filtered.length > 0 ? (
                    filtered.map((c, i) => {
                      ...
                      return (
                        <React.Fragment key={key}>
                          ...
                          >
                            ...
                            <td className="px-6 py-4 text-slate-500 whitespace-nowrap">{formatDate(c.joinedAt)}</td>
            ...
            <div className="md:hidden p-4 space-y-4">
              {loading ? (
                  ...
                  <div key={i} className="bg-white p-4 rounded-lg shadow border animate-pulse">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="h-3 bg-slate-200 w-36 rounded mb-2" />
                        <div className="h-2 bg-slate-200 w-24 rounded" />
                      </div>
                    </div>
                  </div>
                ))
                    ...
                    <div key={key} className="bg-white p-4 rounded-lg shadow border">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3">
                          <Avatar person={c} />
                          <div>
                            <div className="font-medium text-slate-800">{c.fullName || "Unnamed"}</div>
                          ...
                          <AnimatePresence>
                            {openActionFor === key && (
                              <motion.div
                                ...
                                className="absolute right-0 mt-2 w-44 bg-white rounded-lg mobile-action-menu ring-1 ring-slate-100"
                                id={`actions-${key}`}
                              >
                      ...
                      <div className="mt-3 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <button onClick={() => toggleExpand(c)} className="px-3 py-1 rounded bg-slate-100 text-sm hover:bg-slate-200">Expand</button>
                          <Link to={`/customers/vehicles/add?customerId=${encodeURIComponent(key)}`} className="px-3 py-1 rounded bg-emerald-600 text-white text-sm hover:bg-emerald-700">Add Vehicle</Link>
                        </div>
                      </div>
                    </div>
                  );
                ...
                <div className="text-center text-slate-400 p-6">No customers found</div>
              )}
            </div>
          </div>
          ...
          <div className="fixed right-4 bottom-6 md:hidden z-50">
            <Link to="/customers/add">
              <motion.button whileTap={{ scale: 0.95 }} className="w-14 h-14 rounded-full bg-indigo-600 text-white shadow-lg flex items-center justify-center">
                <Icons.Plus size={20} />
              </motion.button>
            </Link>
          </div>
          ...
          {/* Vehicle detail modal (same as before) */}
          <AnimatePresence>
            {selectedVehicle && (
              <motion.div
                className="fixed inset-0 z-50 flex items-center justify-center p-4"
